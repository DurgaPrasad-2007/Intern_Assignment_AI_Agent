...
export class DocumentLoader {
  ...
  private readonly overlapSize: number = 200; // overlap between chunks
   ...
   */
  private parseMarkdownMetadata(content: string, filename: string): {
    title: string;
    author?: string;
    published?: string;
    category: string;
   ...
   */
  private extractPublishedDate(content: string): string | undefined {
  ...
  async getDocumentStats(): Promise<{
    ...
    const categories: Record<string, number> = {};
                  ...
                  if (category) {
                    ...
                    return {
                  totalDocuments: new Set(documents.map(d => d.metadata?.source as string).filter(Boolean)).size,
