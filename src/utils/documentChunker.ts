import type { EnhancedDocument, DocumentChunk, ChunkMetadata } from '../types/index.js';
...
export class DocumentChunker {
  ...
  private detectContentType(text: string): 'text' | 'code' | 'table' | 'list' {
  ...
  private chunkCode(text: string): string[] {
    const chunks: string[] = [];
  ...
  private chunkTable(text: string): string[] {
    const chunks: string[] = [];
  ...
  private chunkList(text: string): string[] {
    const chunks: string[] = [];
  ...
  private splitTextBySize(text: string): string[] {
    const chunks: string[] = [];
  ...
  private fallbackChunking(document: EnhancedDocument): DocumentChunk[] {
    const chunks: DocumentChunk[] = [];
    const text = document.text;
  ...
  extractKeywords(text: string, maxKeywords: number = 10): string[] {
    ...
    for (let i = 0; i < words.length; i++) {
      const word = words[i];
      if (!word) continue; // Skip undefined/null values
