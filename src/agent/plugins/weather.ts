  ...
  private async getWeather(city: string): Promise<string> {
    try {
      ...
      const response = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=0&longitude=0&current=temperature_2m,relative_humidity_2m,weather_code&timezone=auto&city=${encodeURIComponent(city)}`
      );
